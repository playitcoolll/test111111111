...
float AC_PID::update_all_my_y(float target, float measurement, float dt, float aspeed)
{
    ...
    float boost_pitch_p = 1.0f/ 2.0f;
    if(aspeed > 0.0f){
        boost_pitch_p =1.0f/ 2.0f - 1.0f/ 2.0f/20.0f*((abs(aspeed)));
    }
    boost_pitch_p = constrain_float(boost_pitch_p, 0.005f,1.0f);

    float boost_pitch_d = 0.3f/ 2.0f;
    if(aspeed > 0.0f){
        boost_pitch_d  = 0.3f/ 2.0f - 0.3f/ 2.0f/20.0f*((abs(aspeed)));
    }
    boost_pitch_d = constrain_float(boost_pitch_d, 0.005f,0.3f);

    float P_out = (_error * boost_pitch_p);
    float D_out = (_derivative * boost_pitch_d);
    // calculate slew limit modifier for P+D
    _pid_info.Dmod = _slew_limiter.modifier((_pid_info.P + _pid_info.D) * _slew_limit_scale, dt);
