...
float AP_PitchController::_get_rate_out_my(float desired_rate, float scaler, bool disable_integrator, float aspeed, bool ground_mode)
{
    const float dt = AP::scheduler().get_loop_period_s();
...
float AP_PitchController::get_servo_out_my(int32_t angle_err, float scaler, bool disable_integrator, bool ground_mode)
{
    // Calculate offset to pitch rate demand required to maintain pitch angle whilst banking
    // Calculate ideal turn rate from bank angle and airspeed assuming a level coordinated turn
    // Pitch rate offset is the component of turn rate about the pitch axis
    ...
    return _get_rate_out_my(desired_rate, scaler, disable_integrator, aspeed, ground_mode);
}
