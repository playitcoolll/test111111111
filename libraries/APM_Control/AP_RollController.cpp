...
/*
  AC_PID based rate controller
*/
float AP_RollController::_get_rate_out_my(float desired_rate, float scaler, bool disable_integrator, bool ground_mode)
{
    const AP_AHRS &_ahrs = AP::ahrs();
    ...
    // sum components
    float out = pinfo.FF + pinfo.P + pinfo.I*0 + pinfo.D + pinfo.DFF;
    if (ground_mode) {
        // when on ground suppress D term to prevent oscillations
        out -= pinfo.D + 0.5*pinfo.P;
    }
    ...
    // output is scaled to notional centidegrees of deflection
    // return constrain_float(out/57.3f, -45.0f/57.3f, 45.0f/57.3f);
    return constrain_float(out*100, -4500, 4500);
}
...
float AP_RollController::get_servo_out_my(int32_t angle_err, float scaler, bool disable_integrator, bool ground_mode)
{
    // if (gains.tau < 0.05f) {
    //     gains.tau.set(0.05f);
    // }//这个值sitl时候一直是0.5????是自动调参时候用的吗????
    ...
    return _get_rate_out_my(desired_rate, scaler, disable_integrator, ground_mode);
}
