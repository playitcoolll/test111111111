...
void AP_AutoTune::update(AP_PIDInfo &pinfo, float scaler, float angle_err_deg)
{
    ...
    //“在不考虑积分项（I项）的情况下过滤执行器，以便我们可以在不考虑修剪偏移的情况下计算比率。
    //首先需要包括积分项（I），并将其限制在45度以内，以获得实际表面（或执行器）的正确值。”
    //简而言之，这段代码在处理执行器的控制信号时，先计算不包含I项的值，然后通过限制角度来确保执行器的输出在合理范围内（最大为45度
    const float clipped_actuator = constrain_float(pinfo.FF + pinfo.P + pinfo.D + pinfo.DFF + pinfo.I, -45, 45) - pinfo.I;//用于计算控制信号的总和(不加I)
    const float actuator = actuator_filter.apply(clipped_actuator);//平滑信号，减少噪声
    const float actual_rate = rate_filter.apply(pinfo.actual);//平滑信号，减少噪声
    ...
    // update the P and D slew rates, using P and D values from before Dmod was applied更新P和D的变化速率
    const float slew_limit_scale = 45.0 / degrees(1);
    slew_limit_max = rpid.slew_limit();//获取当前的变化速率限制
    slew_limit_tau = 1.0;
    ...
    // thresholds for when we consider an event to start and end定义和计算事件开始和结束的阈值
    const float rate_threshold1 = 0.4 * MIN(att_limit_deg / current.tau.get(), current.rmax_pos);//表示一个速率阈值，通常用于决定飞行器是否处于某种活动状态
    const float rate_threshold2 = 0.25 * rate_threshold1;//用来判断是否达到较低的速率范围，通常用于判断某个事件的结束
    bool in_att_demand = fabsf(angle_err_deg) >= 0.3 * att_limit_deg;//如果误差小于这个值，则认为飞行器处于较小的角度误差范围内
    ...
    if (new_state == state) {
        if (state == ATState::IDLE &&
            now - state_enter_ms > 500 &&//处于 IDLE 状态且持续时间较长且震荡较强时（例如 max_Dmod < 0.9）
            max_Dmod < 0.9) {
    ...
    // adjust P and D调整飞行控制器中的比例（P）和微分（D）增益值
    float D = rpid.kD();
    float P = rpid.kP();
