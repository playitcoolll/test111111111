...
void Plane::update_control_mode(void)
{
    if (control_mode != &mode_auto) {
        ...
        //当飞行控制模式不是“自动模式”（mode_auto）时，将 steer_state.hold_course_cd 设置为 -1，以确保在非自动模式下不保持航向
        steer_state.hold_course_cd = -1;
    }

    update_fly_forward();

    control_mode->update();//注意模式切换用的是遥控器！ 遥控器在哪个挡位control_mode就是什么模式，control_mode只是飞行模式的
    //指针，-> 是调用这个飞行模式中的update（具体例如 *stabilize -> update）
}
...
void Plane::update_fly_forward(void)
{
    ...
    // 当我们纯粹作为固定翼飞机飞行时，确保我们保持前飞状态。
    // 这有助于 EKF（扩展卡尔曼滤波器）生成更好的状态估计，因为它可以做出更强的假设。   
#if HAL_QUADPLANE_ENABLED
    if (quadplane.available() &&
    ...
    if (auto_state.idle_mode) {
        // don't fuse airspeed when in balloon lift 悬停可能类似于悬停或无明显前进方向
        ahrs.set_fly_forward(false);
        return;
    }
